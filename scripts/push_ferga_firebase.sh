...
# config the app uses (~/.config/alpha-ai/config.json) or FIREBASE_API_KEY env.
KAI_CFG="${HOME:-$HOME}/.config/alpha-ai/config.json"
...
if [[ -z "$API_KEY" ]]; then
    echo "❌ No Firebase API key. Set FIREBASE_API_KEY or add ~/.config/alpha-ai/config.json (see config/kurdai.php)."
    exit 1
fi
...
if [[ -f "$CREDS" ]]; then
    ...
    if [[ -z "${FIREBASE_ADMIN_EMAIL:-}" || -z "${FIREBASE_ADMIN_PASSWORD:-}" ]]; then
        ...
        echo "   or set the admin email/password (the alpha-ai.com admin login)."
        exit 1
    fi
    AUTH_MODE="password"
