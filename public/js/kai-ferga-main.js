        ...
        // --- Beam colors per language (Aurora Pro v3 card system) ---
        const BEAM_COLORS = {
            py: ['#22d3ee', '#3b82f6'],
            cpp: ['#818cf8', '#a855f7'],
            js: ['#facc15', '#f59e0b'],
            ...
            rs: ['#fb923c', '#ef4444'],
            cs: ['#c084fc', '#d946ef'],
            'html+css': ['#fb923c', '#ec4899'],
            _default: ['#22d3ee', '#8b5cf6'],
        };
        const AI_GRAD_BEAM = {
            emerald: ['#10b981', '#14b8a6'], teal: ['#14b8a6', '#06b6d4'],
            cyan: ['#22d3ee', '#3b82f6'], indigo: ['#6366f1', '#3b82f6'],
            violet: ['#8b5cf6', '#d946ef'], purple: ['#a855f7', '#d946ef'],
            sky: ['#38bdf8', '#3b82f6'], fuchsia: ['#e879f9', '#ec4899'],
            rose: ['#fb7185', '#f43f5e'], amber: ['#fbbf24', '#f59e0b'],
            blue: ['#3b82f6', '#22d3ee'],
        };
        function beamForLang(l) {
            const ext = (l && l.ext) ? String(l.ext).replace('.', '').toLowerCase() : '';
            return BEAM_COLORS[ext] || BEAM_COLORS._default;
        }
        function beamForAiGrad(grad) {
            const m = /from-(\w+)-/.exec(String(grad || ''));
            return (m && AI_GRAD_BEAM[m[1]]) || BEAM_COLORS._default;
        }
...
print(project("data"))`,
        ...
        // Aurora Pro v3: react to the shared dialect toggle (kurdai-design partial
        // dispatches kai:langchange after rewriting .lang-str nodes) so the
        // dynamically-rendered cards re-render immediately in the new dialect.
        window.addEventListener('kai:langchange', function (e) {
            const next = (e.detail && e.detail.lang) || localStorage.getItem('site-lang') || 'so';
            if (next !== currentLang) {
                currentLang = next;
                applyLanguage();
            }
...
<button class="demo-btn">Click Me</button>
        ...
        onAuthStateChanged(async (user) => { 
            if(!user) {
                ...
                window.isAdmin = ["team@alpha-ai.com", "mahamadkamaran890@gmail.com"].includes(String(user.email || '').toLowerCase());
                if(window.isAdmin) {
        ...
        function glassProgressBar(pct, grad, glow, opts) {
            ...
            const caption = opts.showLabel === false ? '' : `
                <div class="kai-progress-head">
                    <span class="kai-progress-label">${label}</span>
                    <span class="kai-progress-pct ${soon ? '' : 'kai-progress-pct--grad'}">${soon ? '⏳' : p + '%'}</span>
                </div>`;
            const fill = soon
                ? `<div class="kai-progress-fill kai-progress-fill--soon bg-gradient-to-r ${grad}" style="width:38%; --kai-fill-glow:${glow};"></div>`
                : `<div class="kai-progress-fill bg-gradient-to-r ${grad}" style="width:${p}%; --kai-fill-glow:${glow};"></div>`;
            return `
                <div class="kai-progress ${opts.wrapClass || 'kai-progress--card'}" role="progressbar" aria-valuenow="${soon ? 0 : p}" aria-valuemin="0" aria-valuemax="100" aria-label="${label}">
                    ${caption}
                    <div class="kai-progress-track">${fill}</div>
                </div>`;
        }
        ...
        function renderLanguagesGrid() {
            ...
            for (let id in languagesData) {
                ...
                const meta = badgeMetaFor(id);
                const progressBar = glassProgressBar(langPct, meta.grad || 'from-cyan-400 to-blue-600', meta.ring || 'rgba(96,165,250,0.55)', { labelText: `${langDone}/${langTotal} وانە` });
                const iconHtml = l.logo_url
                    ? `<img src="${l.logo_url}" class="kai-cat-logo" alt="${name}">`
                    : `<span class="kai-cat-letter">${name.charAt(0)}</span>`;
                const openAction = needsMembership
                    ? `window.openMembershipModal('${id}')`
                    : `window.openLanguage('${id}')`;
                const lockBadge = showLock ? `
                    <button type="button" onclick="event.stopPropagation(); ${openAction}" class="kai-lock-badge">
                        <svg fill="currentColor" viewBox="0 0 24 24"><path d="M18 8h-1V6a5 5 0 00-10 0v2H6a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V10a2 2 0 00-2-2zm-6 9a2 2 0 110-4 2 2 0 010 4zm3.5-9h-7V6a3.5 3.5 0 117 0v2z"/></svg>
                        ${currentLang === 'so' ? 'بەم زوانە بەردەست دەبێت' : 'بۆ ئەم زوانە بەردەست دبیت'}
                    </button>` : '';
                const doneBadge = badgesEarned[id] ? `
                    <button type="button" onclick="event.stopPropagation(); window.openLanguage('${id}')" class="kai-done-badge">
                        <span>${meta.icon}</span>
                        ${currentLang === 'so' ? 'تەواو بوو' : 'دووماهی بوو'}
                    </button>` : '';
                grid.innerHTML += `
                    <div onclick="${openAction}" class="kai-cat kai-tilt group ${showLock ? 'kai-cat--locked' : ''}"
                         style="--i:${cardIdx++}; --beam-a:${beam[0]}; --beam-b:${beam[1]}; --kai-cat-glow-c:${beam[0]}24;"
                         role="button" tabindex="0" aria-label="${name}">
                        <span class="kai-beam"></span>
                        <span class="kai-cat-glow"></span>
                        ${lockBadge}
                        ${doneBadge}
                        <span class="kai-cat-icon ${l.color || 'bg-blue-100'}">
                            <span class="kai-cat-icon-bg bg-gradient-to-br ${meta.grad || 'from-cyan-400 to-blue-600'}"></span>
                            ${iconHtml}
                        </span>
                        <h3 class="kai-cat-title"><bdi>${name}</bdi></h3>
                        <p class="kai-cat-sub">${desc}</p>
                        ${progressBar}
                        ${window.isAdmin ? `
                        <div class="kai-cat-admin" onclick="event.stopPropagation()">
                            <button type="button" onclick="window.toggleLanguageLock('${id}')" class="kai-mini-btn ${locked ? 'kai-mini-btn--ok' : ''}">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z"></path></svg>
                                ${locked ? (currentLang === 'so' ? 'کردنەوە' : 'ڤەکرن') : (currentLang === 'so' ? 'قفڵکردن' : 'قفڵکرن')}
                            </button>
                            <button type="button" onclick="window.openEditLangModal('${id}')" class="kai-mini-btn kai-mini-btn--warn">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                ${currentLang === 'so' ? 'دەستکاری' : 'دەستکاریکرن'}
                            </button>
                            <button type="button" onclick="window.deleteItem('langs','${id}')" class="kai-mini-btn kai-mini-btn--danger">
        ...
        function renderCategoriesGrid() {
            ...
            // پێشکەوتنی کارتەکان — slim modern bars (per card gradient + glow to match its border)
            const langsCatBar = glassProgressBar(langPctAll, 'from-cyan-400 to-indigo-600', 'rgba(99,102,241,0.55)',
                { labelText: currentLang === 'so' ? `${langDoneAll}/${langTotalAll} وانە تەواو` : `${langDoneAll}/${langTotalAll} وانە دووماهی`, wrapClass: 'kai-progress--card' });
            const aiCatBarAdmin = glassProgressBar(aiPctAll, 'from-emerald-400 to-cyan-500', 'rgba(46,124,255,0.55)',
                { labelText: currentLang === 'so' ? `${aiDoneAll}/${aiTotalAll} وانە تەواو` : `${aiDoneAll}/${aiTotalAll} وانە دووماهی`, wrapClass: 'kai-progress--card' });
            const roboticsCatBar = glassProgressBar(0, 'from-rose-500 to-pink-600', 'rgba(244,63,94,0.55)', { soon: true, wrapClass: 'kai-progress--card' });
            const langPreview = langIds.slice(0, 6).map(id => {
                const l = languagesData[id];
                const name = loc(l, 'name');
                return `<span class="kai-cat-chip ${l.color || 'bg-blue-100'}">${l.logo_url ? `<img src="${l.logo_url}" alt="${name}">` : (name ? name.charAt(0) : '؟')}</span>`;
            }).join('');
            const arrowSvg = `<svg class="kai-cat-arrow" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14 5l7 7-7 7M21 12H3"/></svg>`;
            grid.innerHTML += `
                <div onclick="window.openCategory('langs')" class="kai-cat kai-tilt group h-full"
                     style="--i:0; --beam-a:#22d3ee; --beam-b:#6366f1; --kai-cat-glow-c:rgba(34,211,238,.18);"
                     role="button" tabindex="0">
                    <span class="kai-beam"></span>
                    <span class="kai-cat-glow"></span>
                    <span class="kai-cat-icon kai-cat-icon--xl">
                        <span class="kai-cat-icon-bg bg-gradient-to-br from-blue-500 to-cyan-400"></span>
                        <span class="kai-cat-stack">${langPreview || '<span class="kai-cat-emoji">💻</span>'}</span>
                    </span>
                    <h3 class="kai-cat-title kai-cat-title--lg">${currentLang === 'so' ? 'زمانەکانی پرۆگرامسازی' : 'زمانێن پرۆگرامسازی'}</h3>
                    <p class="kai-cat-sub">${langCount} ${currentLang === 'so' ? 'زمان' : 'زوان'} • ${currentLang === 'so' ? 'بنەڕەتەکانی پرۆگرامسازی فێربە' : 'بنەڕەتێن پرۆگرامسازی فێرببە'}</p>
                    ${langsCatBar}
                    <span class="kai-cat-cta">${currentLang === 'so' ? 'بکەرەوە' : 'ڤەکە'}${arrowSvg}</span>
                </div>
                <div onclick="window.openCategory('ai')" class="kai-cat kai-tilt group h-full"
                     style="--i:1; --beam-a:#10b981; --beam-b:#22d3ee; --kai-cat-glow-c:rgba(16,185,129,.18);"
                     role="button" tabindex="0">
                    <span class="kai-beam"></span>
                    <span class="kai-cat-glow"></span>
                    <span class="kai-cat-icon kai-cat-icon--xl">
                        <span class="kai-cat-icon-bg bg-gradient-to-br from-emerald-400 to-cyan-500"></span>
                        <span class="kai-cat-emoji">🤖</span>
                    </span>
                    <h3 class="kai-cat-title kai-cat-title--lg">${currentLang === 'so' ? 'فێربوونی ژیری دەستکرد' : 'فێربوونا زیرەکیا دەستکرد'}</h3>
                    <p class="kai-cat-sub">${AI_TOPICS.length} ${currentLang === 'so' ? 'بەش' : 'بەش'} • ${AI_TOPICS.reduce((s, t) => s + sortedLangLessons(t.id).length, 0)} ${currentLang === 'so' ? 'وانە' : 'وانە'} • داتا، ئالگۆریتم، ML، DL و LLM</p>
                    ${window.isAdmin ? aiCatBarAdmin : ''}
                    <span class="kai-cat-cta">${currentLang === 'so' ? 'بکەرەوە' : 'ڤەکە'}${arrowSvg}</span>
                </div>
                <div onclick="window.openRoboticsComingSoon()" class="kai-cat kai-cat--soon h-full"
                     style="--i:2; --beam-a:#f43f5e; --beam-b:#ec4899; --kai-cat-glow-c:rgba(244,63,94,.14);">
                    <span class="kai-beam"></span>
                    <span class="kai-cat-glow"></span>
                    <span class="kai-cat-icon kai-cat-icon--xl">
                        <span class="kai-cat-icon-bg bg-gradient-to-br from-rose-500 via-red-500 to-pink-600"></span>
                        <span class="kai-cat-emoji">🦾</span>
                    </span>
                    <h3 class="kai-cat-title kai-cat-title--lg">${currentLang === 'so' ? 'ڕۆبۆتیک' : 'ڕۆبۆتیک'}</h3>
                    <p class="kai-cat-sub">${currentLang === 'so' ? 'بەمزووانە بەردەست دەبێت' : 'د نزیکترین دەمی دا بەردەست دبیت'} • ${currentLang === 'so' ? 'ڕۆبۆت، سینسۆر، مایکرۆکۆنترۆلەر و پرۆگرامسازی ڕۆبۆتەکان' : 'ڕۆبۆت، سینسۆر، مایکرۆکۆنترۆلەر و بەرنامەکیرنا ڕۆبۆتان'}</p>
                    ${roboticsCatBar}
                    <span class="kai-cat-cta kai-cat-cta--soon">${currentLang === 'so' ? 'بەم زوانە بەردەست دەبێت' : 'د ڤێ زوانێ دا دێ بەردەست بیت'}</span>
                </div>`;
        }
        ...
        function renderAITopicsGrid() {
            ...
            topics.forEach((t, ti) => {
                ...
                const stateBadge = isDone
                    ? `<span class="kai-badge kai-badge--done"><span>${badgeMetaFor(t.id).icon}</span>${currentLang === 'so' ? 'تەواو بوو' : 'دووماهی بوو'}</span>`
                    : (!logicallyLocked
                        ? `<span class="kai-badge kai-badge--ok">🔓 ${currentLang === 'so' ? 'کراوە' : 'ڤەکری'}</span>`
                        : `<span class="kai-badge kai-badge--locked"><svg fill="currentColor" viewBox="0 0 24 24"><path d="M18 8h-1V6a5 5 0 00-10 0v2H6a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V10a2 2 0 00-2-2zm-6 9a2 2 0 110-4 2 2 0 010 4zm3.5-9h-7V6a3.5 3.5 0 117 0v2z"/></svg>${currentLang === 'so' ? 'قفڵکراوە' : 'قفڵکری'}</span>`);
                ...
                const iconHtml = t.logo_url
                    ? `<img src="${t.logo_url}" class="kai-cat-logo" alt="${name}">`
                    : `<span class="kai-cat-emoji">${t.icon || '🤖'}</span>`;
                grid.innerHTML += `
                    <div onclick="${openAction}" class="kai-cat kai-tilt group ${isUnlocked ? '' : 'kai-cat--locked'}"
                         style="--i:${ti}; --beam-a:${beam[0]}; --beam-b:${beam[1]}; --kai-cat-glow-c:${beam[0]}1f;"
                         role="button" tabindex="0" aria-label="${name}">
                        <span class="kai-beam"></span>
                        <span class="kai-cat-glow"></span>
                        ${stateBadge}
                        <span class="kai-cat-icon" ${window.isAdmin ? `onclick="event.stopPropagation(); window.changeAILogo('${t.id}')"` : ''}>
                            <span class="kai-cat-icon-bg bg-gradient-to-br ${grad}"></span>
                            ${iconHtml}
                            <span class="kai-cat-num">${ti + 1}</span>
                            ${logicallyLocked ? `<span class="kai-cat-icon-lock"><svg class="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M18 8h-1V6a5 5 0 00-10 0v2H6a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V10a2 2 0 00-2-2zm-6 9a2 2 0 110-4 2 2 0 010 4zm3.5-9h-7V6a3.5 3.5 0 117 0v2z"/></svg></span>` : ''}
                        </span>
                        <h3 class="kai-cat-title"><bdi>${name}</bdi></h3>
                        <span class="kai-cat-count">${completed}/${total}</span>
                        <p class="kai-cat-sub">${desc}</p>
                        <div class="kai-progress-track kai-progress-track--slim w-full">
                            <div class="kai-progress-fill bg-gradient-to-r ${grad}" style="width:${pct}%; --kai-fill-glow:rgba(93,155,255,0.55)"></div>
                        </div>
                        ${logicallyLocked ? `<span class="kai-cat-soon-note">🔒 ${currentLang === 'so' ? 'بەم زوانە بەردەست دەبێت' : 'د ڤێ زوانێ دا دێ بەردەست بیت'}</span>` : ''}
                        <span class="kai-cat-cta ${isUnlocked ? '' : 'kai-cat-cta--soon'}">${btnText}${arrowSvg}</span>
                        ${window.isAdmin ? `
                        <div class="kai-cat-admin" onclick="event.stopPropagation()">
                            <button type="button" onclick="window.toggleAILock('${t.id}')" class="kai-mini-btn ${!openToAll ? 'kai-mini-btn--ok' : 'kai-mini-btn--danger'}">
                                ${!openToAll ? (currentLang === 'so' ? 'کردنەوە' : 'ڤەکە') : (currentLang === 'so' ? 'قفڵکردن' : 'قفڵکرن')}
                            </button>
                            <button type="button" onclick="window.openNewLessonModal('${t.id}')" class="kai-mini-btn kai-mini-btn--ok">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path></svg>
                                ${currentLang === 'so' ? 'وانە' : 'وانە'}
                            </button>
                            <button type="button" onclick="window.openEditLangModal('${t.id}')" class="kai-mini-btn kai-mini-btn--warn">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                ${currentLang === 'so' ? 'دەستکاری' : 'دەستکاریکرن'}
                            </button>
                        </div>` : ''}
