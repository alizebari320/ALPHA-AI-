...
@section('title', __('AlphaAi — AI, made understandable'))
...
<div class="alpha-home">
    <section class="hero-signal" aria-labelledby="hero-title">
        ...
        <div class="container hero-inner">
            <div class="hero-copy reveal">
                <p class="hero-badge"><span class="pulse-dot"></span> {{ __('Signal Feed') }}</p>
                <h1 id="hero-title">{{ __('The signal') }}<br><em>{{ __('behind') }}</em> {{ __('the noise.') }}</h1>
            ...
            <div class="hero-console reveal stagger-2" aria-label="AlphaAi live platform summary">
                <div class="console-top"><span>ALPHAAI / {{ __('LIVE INDEX') }}</span><span class="console-status"><i></i> {{ __('ONLINE') }}</span></div>
                <div class="console-ring"><div class="ring-core"><span>AI</span><small>{{ __('DISCOVER') }}<br>{{ __('WHAT\'S NEXT') }}</small></div></div>
                <div class="console-data"><div><strong>{{ count($tools ?? []) ?: '200' }}</strong><span>{{ __('tools indexed') }}</span></div><div><strong>04</strong><span>{{ __('languages') }}</span></div><div><strong>{{ count($courses ?? []) ?: '12' }}</strong><span>{{ __('learning paths') }}</span></div></div>
    ...
    <section class="section signal-section" aria-labelledby="explore-title">
        <div class="container">
            <div class="section-heading"><div><p class="eyebrow">{{ __('One platform. Many directions.') }}</p><h2 id="explore-title">{{ __('Find your next') }} <span>{{ __('advantage.') }}</span></h2></div><p>{{ __('A focused space for learning, building, and staying ahead of the curve.') }}</p></div>
            <div class="category-grid bento-grid">
    ...
    <section class="stats-band"><div class="container stats-grid"><div class="stat-card"><strong>{{ count($tools ?? []) ?: '200' }}</strong><span>{{ __('Tools indexed') }}</span></div><div class="stat-card"><strong>04</strong><span>{{ __('Languages') }}</span></div><div class="stat-card"><strong>{{ count($courses ?? []) ?: '50' }}</strong><span>{{ __('Learning paths') }}</span></div><div class="stat-card"><strong>24/7</strong><span>{{ __('Always available') }}</span></div></div></section>
