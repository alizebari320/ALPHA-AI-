...
@section('content')
    ...
    @component('partials.auth-card')
        ...
        @slot('body')
            ...
            <form method="POST" action="#" class="space-y-4" x-data="authForm()" @submit.prevent="login()">
                <div>
                    <label for="email" class="label">ئیمێڵ</label>
                    <input type="email" id="email" name="email" x-model="email" placeholder="ئیمێڵەکەت بنووسە" class="field text-left" dir="ltr" autocomplete="email" @keydown.enter="login">
                </div>
                <div>
                    <label for="password" class="label">وشەی نهێنی</label>
                    <div class="relative">
                        <input type="password" id="password" name="password" x-model="password" placeholder="وشەی نهێنی (لانی کەم ٦ پیت)" class="field text-left pr-12" dir="ltr" autocomplete="current-password" @keydown.enter="login">
