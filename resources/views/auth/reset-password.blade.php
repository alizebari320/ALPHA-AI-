...
@section('title', 'گۆڕینی وشەی نهێنی — ALPHA/AI')
...
@section('content')
    ...
    @component('partials.auth-card')
        @slot('header')
            ...
            <h2 class="font-mega text-3xl tracking-wide text-zinc-100 mb-1">گۆڕینی وشەی نهێنی</h2>
            <p class="text-zinc-500 font-mono text-xs tracking-widest mt-2">وشەیەکی نوێ هەڵبژێرە بۆ هەژمارەکەت</p>
        @endslot

        @slot('body')
            <form method="POST" action="{{ route('password.store') }}" x-data="resetForm()" @submit.prevent="submit">
                @csrf
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="email" id="email" name="email" x-model="email" class="field text-left" dir="ltr" value="{{ old('email', $request->email) }}" required autofocus autocomplete="email">
                        <x-input-error :messages="$errors->get('email')" class="mt-1.5 text-sm text-rose-400" />
                    </div>

                    <div>
                        <label for="password" class="label">وشەی نهێنی نوێ</label>
                        <div class="relative">
                ...
                <div class="mt-6">
                    <button type="submit" class="w-full btn btn-primary py-3.5 text-sm" :disabled="loading">
                        <span x-show="!loading">گۆڕینی وشەی نهێنی</span>
